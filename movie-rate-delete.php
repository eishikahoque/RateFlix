	...
	<body>
		<?php if (isset($_SESSION['userID'])){ 
			...
			<section class="review-delete">
				<h2>Are you sure you want to delete your movie rating?</h2>
				<?php 
				$ratingID = $_GET["ratingID"];
				$stmt = $pdo->prepare("SELECT * FROM `movies-rating` WHERE `ratingID` = '$ratingID';");
				$stmt->execute();?>
				<div class="account-reviews">
					<?php $row = $stmt->fetch();
						?>
							...
							<form action="movie-rate-process-delete.php" method="POST">
