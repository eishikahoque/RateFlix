...
<body>
  ...
  <main class="main-container">
    ...
    <section class="align-middle-fixed">
      <div class="first-line">
        ...
        <div class="second-line">
          ...
          <?php
            $rateAvg = $pdo->prepare("SELECT AVG(`myRating`) as avrg FROM `movies-rating` WHERE `movieID` = '$movieID'");
            $rateAvg->execute();
      ...
      <h3>Reviews</h3>
        <?php			
        $stmt = $pdo->prepare("SELECT * FROM `movies-review` WHERE `movieID`='$movieID' ");
        $stmt->execute();?>
    ...
    <section class="mobile-section">
      ...
      <?php			
				$stmt = $pdo->prepare("SELECT * FROM `movies-rating` WHERE `movieID`='$movieID' AND `userID` ='$userID'");
				$stmt->execute();
				$row = $stmt->fetch();
      ?>
      ...
      <?php			
				$stmt = $pdo->prepare("SELECT * FROM `movies-review` WHERE `movieID`='$movieID' AND `userID` ='$userID'");
				$stmt->execute();
				$row = $stmt->fetch();
      ?>
      ...
      <?php
        $stmt = $pdo->prepare("SELECT * FROM `movies` ORDER BY RAND() LIMIT 3");
        $stmt->execute();?>
        <div class="tile">
        <?php while($row = $stmt->fetch()){ ?>
